...
locations.each {|location|
    ...
    p "#{Time.now} - #{frames.count} records for location #{location} loaded in #{(start - Time.now).abs} seconds"
}
...
get '/locations' do
    JSON.generate CACHE.keys
end

get '/historics/:location' do
  historics = calculate_historics(frames)
